...
import { generarEmparejamientosIndividuales } from '../funcionesDracula/emparejamientosIndividualesDracula';

import ModalRegistroPartidaDracula from '../ModalRegistroPartidaDracula';
...
function VistaEmparejamientosDracula({ torneoId: propTorneoId, esVistaPublica = false }) {
    ...
    const cargarDatos = async () => {
        try {
            ...
            const responseTorneo = await torneosDraculaApi.obtenerTorneo(torneoId);
            const dataTorneo = responseTorneo.data?.torneo || responseTorneo.torneo || responseTorneo;
            setTorneo(dataTorneo);

            const responseJugadores = await torneosDraculaApi.obtenerJugadoresTorneo(torneoId);
    ...
    const cargarTodasLasPartidas = async (tId = torneoId) => {
        try {
            const response = await torneosDraculaApi.obtenerPartidasTorneo(tId);
    ...
    const cargarPartidasRonda = async (tId, ronda) => {
        ...
        try {
            ...
            const response = await torneosDraculaApi.obtenerEmparejamientosIndividuales(idFinal, rondaFinal);
            const partidas = response?.data || response || [];
    ...
    const handleGenerarEmparejamientos = async () => {
        try {
            ...
            const responseClasificacion = await torneosDraculaApi.obtenerClasificacionIndividual(torneoId);
            ...
            const participantes = jugadores.map(j => {
                ...
                return {
                    ...j,
                    puntos_ronda: stats?.puntos_ronda_totales || 0,
        ...
        nuevosEmp[emparejamientoEditando.index] = {
            ...
            jugador1: jugador1
                ? {
                    ...
                    club: jugador1.club || '-',
                    banda: jugador1.banda || '-',
                    puntos_ronda: jugador1.puntos_ronda || 0,
            ...
            jugador2: jugador2
                ? {
                    ...
                    club: jugador2.club || '-',
                    banda: jugador2.banda || '-',
                    puntos_ronda: jugador2.puntos_ronda || 0,
    ...
    const guardarResultados = async () => {
        ...
        try {
            ...
            await torneosDraculaApi.guardarEmparejamientosIndividuales(
                torneo.id,
                partidasAGuardar,
                torneo.ronda_actual
            );
    ...
    const generarSiguienteRonda = async () => {
        try {
            ...
            await torneosDraculaApi.actualizarTorneo(torneo.id, {
    ...
    const todasLasPartidasCompletas = () => {
        ...
        return partidasGuardadas.every(partida =>
            partida.resultado_pd &&
            partida.resultado_pd !== 'pendiente' &&
            partida.resultado_pd !== null
        );
    };
    ...
    const confirmarPartida = async (partidaId, confirmar) => {
        try {
            await torneosDraculaApi.confirmarResultado(torneo.id, partidaId, confirmar);
            alert(confirmar
    ...
    const renderPartidaIndividual = (partida, index, esRondaActual) => {
        ...
        const j1Banda = partida.jugador1_banda  || partida.jugador1?.banda || null;
        ...
        const j2Banda = partida.jugador2_banda  || partida.jugador2?.banda || null;
            ...
            >
                ...
                <div className="enfrentamiento">
                    <div className="jugador">
                        ...
                        {j1Banda && (
                            <div className="faccion">⚔️ {j1Banda}</div>
                        )}
                        <div className="stats">
                            PV: {parseFloat(partida.puntos_ronda_j1 || 0).toFixed(1)} |
                            PM: {parseFloat(partida.puntos_victoria_j1 || 0).toFixed(1)}
                        </div>
                    </div>
                    ...
                    {j2Nombre ? (
                        <div className="jugador">
                            ...
                            {j2Banda && (
                                <div className="faccion">⚔️ {j2Banda}</div>
                            )}
                            <div className="stats">
                                PV: {parseFloat(partida.puntos_ronda_j2 || 0).toFixed(1)} |
                                PM: {parseFloat(partida.puntos_victoria_j2 || 0).toFixed(1)}
                            </div>
                        </div>
                    ) : (
                                            ...
                                            <div key={index} className="emparejamiento-card">
                                                ...
                                                <div className="enfrentamiento">
                                                    <div className="jugador">
                                                        ...
                                                        {emp.jugador1?.banda && (
                                                            <div className="faccion">⚔️ {emp.jugador1.banda}</div>
                                                        )}
                                                    </div>
                                                    <div className="vs">VS</div>
                                                    <div className="jugador">
                                                        ...
                                                        {emp.jugador2?.banda && (
                                                            <div className="faccion">⚔️ {emp.jugador2.banda}</div>
                                                        )}
                                                    </div>
                                                </div>
                                            </div>
...
export default VistaEmparejamientosDracula;