...
function VistaGeneralDracula({ torneoId: propTorneoId, onUpdate }) {
    ...
    const [datosEdicion, setDatosEdicion] = useState({
        ...
        puntos_banda: PUNTOS_BANDA_DRACULA.default,
    ...
    useEffect(() => {
        if (torneo) {
            ...
            setDatosEdicion({
                ...
                puntos_banda: torneo.puntos_banda || PUNTOS_BANDA_DRACULA.default,
    ...
    const cargarDatos = async () => {
        try {
            ...
            const response = await torneosDraculaApi.obtenerTorneo(torneoId);
            const dataTorneo = response.data?.torneo || response.torneo || response;
            setTorneo(dataTorneo);
            
            try {
                const dataJugadores = await torneosDraculaApi.obtenerJugadoresTorneo(torneoId);
    ...
    const cargarOrganizadores = async () => {
        try {
            const data = await torneosDraculaApi.obtenerOrganizadores(torneoId);
    ...
    const handleReenviarInvitacion = async (org) => {
        ...
        if (window.confirm(`¿Reenviar invitación a ${org.email}?`)) {
          try {
            await torneosDraculaApi.reenviarInvitacion(torneo.id, org.organizador_id);
    ...
    const handleGuardarCambios = async (e) => {
        ...
        try {
            ...
            await torneosDraculaApi.actualizarTorneo(torneoId, dataToSend);
    ...
    const cambiarEstadoTorneo = async (nuevoEstado) => {
        ...
        if (nuevoEstado === 'en_curso') {
            try {
                const jugadoresData = await torneosDraculaApi.obtenerJugadoresTorneo(torneoId);
                ...
                const response = await torneosDraculaApi.verificarPagos(torneoId);
        ...
        try {
            await torneosDraculaApi.cambiarEstadoTorneo(torneoId, nuevoEstado);
    ...
    const eliminarTorneo = async () => {
        ...
        try {
            await torneosDraculaApi.eliminarTorneo(torneoId);
    ...
    const descargarBases = async () => {
        try {
            await torneosDraculaApi.descargarBasesPDF(torneoId);
        } catch (error) {
            console.error('Error:', error);
    ...
    const handleAgregarOrganizador = async (e) => {
        ...
        try {
            ...
            const response = await torneosDraculaApi.agregarOrganizador(torneoId, {
    ...
    const handleEliminarOrganizador = async (organizadorId, tipo, nombre) => {
        ...
        try {
            ...
            await torneosDraculaApi.eliminarOrganizador(torneoId, organizadorId);
                ...
                <form onSubmit={handleGuardarCambios} className="formulario-edicion">
                    <fieldset>
                        ...
                        <div className="form-row">
                            ...
                            <div className="form-group">
                                <label htmlFor="puntos_ejercito">Puntos Bandas:*</label>
                                <input
                                    type="number"
                                    id="puntos_ejercito"
                                    ...
                                    disabled={loadingEdicion}
                                />
                                <small>{PUNTOS_BANDA_DRACULA.min}-{PUNTOS_BANDA_DRACULA.max} pts</small>
                            </div>
                        ...
                        <select
                            ...
                            disabled={loadingEdicion}
                        >
                            {ESTADOS_TORNEO_DRACULA.map(estado => (
                ...
                <>
                    <section className="seccion-info-torneo">
                        ...
                        <div className="info-torneo-grid">
                            ...
                            <div className="info-item">
                                <label>⚔️ Puntos de las Bandas:</label>
...
export default VistaGeneralDracula;