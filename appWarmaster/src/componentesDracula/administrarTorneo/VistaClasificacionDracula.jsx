...
function VistaClasificacionDracula({ torneoId: propTorneoId }) {
    ...
    const cargarClasificacionIndividual = async () => {
        try {
            const response = await torneosDraculaApi.obtenerClasificacionIndividual(torneoId);
            ...
            const clasificacionOrdenada = dataClasificacion.sort((a, b) => {
                if (b.puntos_ronda_totales !== a.puntos_ronda_totales) {
                    return b.puntos_ronda_totales - a.puntos_ronda_totales;
                }
            ...
            <tbody>
                {clasificacionIndividual.map((jugador, index) => (
                    <tr key={`jugador-${jugador.jugador_id || index}-${jugador.torneo_id || torneoId}`} className={index < 3 ? `top-${index + 1}` : ''}>
                        ...
                        <td>{jugador.club || '-'}</td>
                        <td>{jugador.banda && jugador.nombre_banda || '-'}</td>
                        ...
                        <td>{jugador.puntos_victoria_totales || 0}</td>
                        <td className="puntos-destacado">{jugador.puntos_ronda_totales || 0}</td>
                    </tr>
                ))}
            </tbody>
        </table>
...
export default VistaClasificacionDracula;