...
function PrincipalDracula({ onOpenLogin }) {
    const navigate = useNavigate();
    const { isAuthenticated, user} = useAuth();

    const userId = user?.id || null;
    const [torneosDracula, setTorneosDracula] = useState([])

    const [loading, setLoading] = useState(true);
    const [error, setError] = useState('');
 
    
    const obtenerTorneosDracula = async () => {
        try {
            setLoading(true);
            setError('');
           ...
           const token = localStorage.getItem('token');
            ...
            const data = await torneosDraculaApi.obtenerTorneos()
            
            if (data.data && data.data.torneosDracula) {
                setTorneosDracula(data.data.torneosDracula);
            } else {
    ...
    return (
        <div>     
            ...
            <section>
                <img src={logoDracula} alt="logo de DRACULA"  />
                <p>Consulta los torneos de DRACULA´S AMÉRICA en la Península.</p>
                
                {loading ? (
                    <p className="loading-message">⏳ Cargando torneos...</p>
                ) : torneosDracula.length === 0 ? (
                    ...
                    <div className="tabla-container">
                        <table className="tabla-torneos">
                            ...
                            <tbody>
                                {torneosDracula.map((torneo, index) => (
                                    <tr key={torneo.id}>
                                        ...
                                        <td className="acciones-cell">
                                                ...
                                                <button 
                                                    ...
                                                    onClick={() => {
                                                        if (torneo.usuario_inscrito) {
                                                            navigate(`/torneosDracula/${torneo.id}/editar-inscripcion`);
                                                        } else {
                                            ...
                                            <button 
                                                ...
                                                onClick={() => navigate(`/torneosDracula/${torneo.id}/detalles`)}
                                            >   
                                                👁️ Ver Detalles
                                            </button>
                                        </td>
...
export default PrincipalDracula;