// componentesEpic/inscripcionEpicIndividual.jsx

import React, { useState, useEffect } from "react";
import { useNavigate, useLocation } from "react-router-dom";

import torneosDraculaApi from '@/servicios/apiDracula.js';
import { BANDAS_DRACULA } from '@/componentesDracula/funcionesDracula/constantesFuncionesDracula';
import Footer from '@/paginas/Footer.jsx'

import '@/estilos/inscripcion.css';

function InscripcionDraculaIndividual({ torneoId, torneo, user }) {
  const navigate = useNavigate();
  const location = useLocation();
  
  // Detectar si es modo edición
  const [ modoEdicion, setModoEdicion] = useState( 
    location.pathname.includes('editar-inscripcion') || 
    location.pathname.includes('actualizarInscripcion')
  );
  
  // Estados
  const [nombreBanda, setNombreBanda] = useState(""); // ✅ NUEVO ESTADO
  ...
  // ==========================================
    ...
    useEffect(() => {
      const cargarInscripcion = async () => {
        try {
          setLoading(true);
          const dataInscripcion = await torneosDraculaApi.obtenerIncripcion(torneoId);
          
          if (dataInscripcion.success && dataInscripcion.data) {
            const inscripcion = dataInscripcion.data;
            
            // ✅ Si tiene inscripción, activar modo edición automáticamente
            setModoEdicion(true);
            setNombreBanda(inscripcion.nombre_banda || "");
            setBandaSeleccionada(inscripcion.banda || "");
          ...
          // 404 = no inscrito, es normal, no hacer nada
      ...
      cargarInscripcion();
    }, [torneoId]); // ← ya no depende de modoEdicion
  ...
  const handleVerPDF = async () => {
    ...
    try {
      setLoadingPdf(true);
      setError("");
      await torneosDraculaApi.verListaEjercito(torneoId, user.id);
    } catch (err) {
  ...
  const handleDescargarPDF = async () => {
    ...
    try {
      setLoadingPdf(true);
      setError("");
      await torneosDraculaApi.descargarListaEjercito(torneoId, user.id);
    } catch (err) {
  ...
  const eliminarInscripcion = async () => {
    ...
    try {
      setLoading(true);
      const resultado = await torneosDraculaApi.eliminarJugadorTorneo(torneoId, user.id);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!nombreBanda.trim()) { // ✅ VALIDAR NOMBRE
      setError("Debes indicar el nombre de tu banda");
      return;
    }
    ...
    try {
      ...
      if (archivoPDF) {
        ...
        inscripcionData.append('nombre_banda', nombreBanda.trim()); // ✅ AGREGAR NOMBRE
        inscripcionData.append('banda', bandaSeleccionada);
        inscripcionData.append('lista_banda', archivoPDF);
      } else {
        inscripcionData = {
          nombre_banda: nombreBanda.trim(), // ✅ AGREGAR NOMBRE
      ...
      if (modoEdicion) {
        resultado = await torneosDraculaApi.actualizarInscripcion(torneoId, inscripcionData);
        alert("✅ ¡Inscripción actualizada con éxito!");
      } else {
        resultado = await torneosDraculaApi.inscribirse(torneoId, inscripcionData);
  ...
  const puntosMaximos = torneo?.puntos_banda || 100;
  ...
  return (
    <div className="inscripcion-container">
      ...
      <section className="info-torneo">
        ...
        <div className="datos-grid">
          <div className="dato-item">
            <label>Puntos Máximos de la Banda:</label>
      ...
      <form onSubmit={handleSubmit} className="inscripcion-form">
        ...
        <fieldset>
          <legend>⚔️ Datos de la Banda</legend>
          
          {/* ✅ NUEVO CAMPO: NOMBRE DE LA BANDA */}
          <div className="form-group">
            <label htmlFor="nombreEjercito">Nombre de la Banda:*</label>
            <input
              type="text"
              id="nombreEjercito"
              value={nombreBanda}
              onChange={(e) => setNombreBanda(e.target.value)}
              placeholder="Ej: Fuerza Expedicionaria XXXIII"
              maxLength={100}
              required
              disabled={loading}
            />
          ...
          <div className="form-group">
            ...
            >
              ...
              {BANDAS_DRACULA.map((banda, index) => (
        ...
        {/* SUBIR LISTA DE LA BANDA (OPCIONAL) */}
        <fieldset>
          <legend>📄 Lista de la Banda (Opcional)</legend>
        ...
        <div className="button-group">
          <button 
            ...
            disabled={loading || !bandaSeleccionada || !nombreBanda.trim()}
          >
            {loading 
              ? '⏳ Procesando...' 
...
export default InscripcionDraculaIndividual;