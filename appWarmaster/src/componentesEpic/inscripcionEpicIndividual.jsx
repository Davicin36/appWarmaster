...
import { EJERCITOS_EPIC } from '@/componentesEpic/funcionesEpic/constantesFuncionesEpic';
import Footer from '@/paginas/Footer.jsx'

import '@/estilos/inscripcion.css';

function InscripcionEpicIndividual({ torneoId, torneo, user }) {
  ...
  // ==========================================
    ...
    useEffect(() => {
      const cargarInscripcion = async () => {
        try {
          setLoading(true);
          const dataInscripcion = await torneosEpicApi.obtenerIncripcion(torneoId);
  ...
  const handleVerPDF = async () => {
    ...
    try {
      setLoadingPdf(true);
      setError("");
      await torneosEpicApi.verListaEjercito(torneoId, user.id);
    } catch (err) {
  ...
  const handleDescargarPDF = async () => {
    ...
    try {
      setLoadingPdf(true);
      setError("");
      await torneosEpicApi.descargarListaEjercito(torneoId, user.id);
    } catch (err) {
  ...
  const eliminarInscripcion = async () => {
    ...
    try {
      setLoading(true);
      const resultado = await torneosEpicApi.eliminarJugadorTorneo(torneoId, user.id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modoEdicion) {
        resultado = await torneosEpicApi.actualizarInscripcion(torneoId, inscripcionData);
        alert("✅ ¡Inscripción actualizada con éxito!");
      } else {
        resultado = await torneosEpicApi.inscribirse(torneoId, inscripcionData);
        ...
        <fieldset>
          ...
          <div className="form-group">
            ...
            >
              ...
              {EJERCITOS_EPIC.map((ejercito, index) => (
...
export default InscripcionEpicIndividual;