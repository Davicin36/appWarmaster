...
function VistaGeneralEpic({ torneoId: propTorneoId, onUpdate }) {
    ...
    const [datosEdicion, setDatosEdicion] = useState({
        ...
        puntos_ejercito: PUNTOS_EJERCITO_EPIC.default,
    ...
    useEffect(() => {
        if (torneo) {
            ...
            setDatosEdicion({
                ...
                puntos_ejercito: torneo.puntos_ejercito || PUNTOS_EJERCITO_EPIC.default,
    ...
    const cargarDatos = async () => {
        try {
            ...
            const response = await torneosEpicApi.obtenerTorneo(torneoId);
    ...
    const cargarOrganizadores = async () => {
        try {
            const data = await torneosEpicApi.obtenerOrganizadores(torneoId);
    ...
    const handleReenviarInvitacion = async (org) => {
        ...
        if (window.confirm(`¿Reenviar invitación a ${org.email}?`)) {
          try {
            await torneosEpicApi.reenviarInvitacion(torneo.id, org.organizador_id);
    ...
    const handleGuardarCambios = async (e) => {
        ...
        try {
            ...
            await torneosEpicApi.actualizarTorneo(torneoId, dataToSend);
    ...
    const cambiarEstadoTorneo = async (nuevoEstado) => {
        ...
        if (nuevoEstado === 'en_curso') {
            try {
                const jugadoresData = await torneosEpicApi.obtenerJugadoresTorneo(torneoId);
                ...
                const response = await torneosEpicApi.verificarPagos(torneoId);
        ...
        try {
            await torneosEpicApi.cambiarEstadoTorneo(torneoId, nuevoEstado);
    ...
    const eliminarTorneo = async () => {
        ...
        try {
            await torneosEpicApi.eliminarTorneo(torneoId);
    ...
    const descargarBases = async () => {
        try {
            await torneosEpicApi.descargarBasesPDF(torneoId);
        } catch (error) {
            console.error('Error:', error);
    ...
    const handleAgregarOrganizador = async (e) => {
        ...
        try {
            ...
            const response = await torneosEpicApi.agregarOrganizador(torneoId, {
    ...
    const handleEliminarOrganizador = async (organizadorId, tipo, nombre) => {
        ...
        try {
            ...
            await torneosEpicApi.eliminarOrganizador(torneoId, organizadorId);
                ...
                <form onSubmit={handleGuardarCambios} className="formulario-edicion">
                    <fieldset>
                        ...
                        <div className="form-row">
                            ...
                            <div className="form-group">
                                ...
                                <input
                                    ...
                                    disabled={loadingEdicion}
                                />
                                <small>{PUNTOS_EJERCITO_EPIC.min}-{PUNTOS_EJERCITO_EPIC.max} pts</small>
                            </div>
                        ...
                        <select
                            ...
                            disabled={loadingEdicion}
                        >
                            {ESTADOS_TORNEO_EPIC.map(estado => (
...
export default VistaGeneralEpic;