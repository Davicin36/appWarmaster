...
//==============================
//RUTAS TORNEOS DRACULAS AMERICA
//==============================
...
router.get('/obtenerTorneos', async (req, res) => {
  try {
    console.log('📥 GET /api/torneosDracula/obtenerTorneos');
    ...
    let whereClause = 'WHERE ts.sistema = "DRACULA"';
    ...
    const [torneos] = await pool.query(`
      SELECT 
        ...
        COUNT(DISTINCT jtd.id) as total_participantes,
        MAX(CASE WHEN jtd.jugador_id = ? THEN 1 ELSE 0 END) as usuario_inscrito,
      ...
      LEFT JOIN jugador_torneo_dracula jtd ON ts.id = jtd.torneo_id
    ...
    let countParams = [];
    let countWhereClause = 'WHERE ts.sistema = "DRACULA"';
    ...
    console.error('❌ Error al obtener torneos Dracula:', error);
  ...
  try {
    ...
    const [torneos] = await pool.execute(`
      SELECT 
        ...
        u.club as creador_club,
        COUNT(DISTINCT CASE WHEN ts.tipo_torneo = 'Individual' THEN jtd.id ELSE NULL END) as total_participantes,
        MAX(CASE WHEN jtd.jugador_id = ? THEN 1 ELSE 0 END) as usuario_inscrito,
        MAX(CASE WHEN co.usuario_id = ? THEN 1 ELSE 0 END) as es_coorganizador
      FROM torneos_sistemas ts 
      LEFT JOIN usuarios u ON ts.created_by = u.id 
      LEFT JOIN jugador_torneo_dracula jtd ON ts.id = jtd.torneo_id
      LEFT JOIN organizadores_torneos co ON ts.id = co.torneo_id
      WHERE ts.id = ? AND ts.sistema="DRACULA"
      GROUP BY ts.id
...
router.post('/creandoTorneo', verificarToken,uploadMultiple.fields([
  ...
  try {
    ...
    if (puntos_banda < 50 || puntos_banda > 200) {
      return res.status(400).json(
        errorResponse('Los puntos de banda deben estar entre 50 y 200')
      );
    }
...
router.put('/:torneoId/actualizarTorneo', verificarToken, verificarOrganizadorTorneo, uploadMultiple.fields([
  ...
  try {
    ...
    if (puntos_ejercito && (puntos_ejercito < 50 || puntos_ejercito > 200)) {
      return res.status(400).json(
        errorResponse('Los puntos de ejército deben estar entre 50 y 200')
      );
    }
...
router.post('/:torneoId/inscripcion', verificarToken, upload.single('lista_ejercito'), async (req, res) => {
  try {
    ...
    const { nombre_banda, banda } = req.body;

    if(!nombre_banda || !nombre_banda.trim()) {
      return  res.status(400).json(
        errorResponse('El nombre de la banda es obligatorio')
      );
    } 

     if (!banda || !banda.trim()) {
      return res.status(400).json(
        errorResponse('La banda es obligatoria')
      );
    }
    ...
    const [inscripcionExistente] = await pool.execute(
      'SELECT id FROM jugador_torneo_dracula WHERE torneo_id = ? AND jugador_id = ?',
      [torneoId, usuarioId]
    );
    ...
    const [resultado] = await pool.execute(
      `INSERT INTO jugador_torneo_dracula (
          torneo_id, 
          jugador_id, 
          nombre_banda,
          banda,
...
router.get('/:torneoId/obtenerInscripcion', verificarToken, async (req, res) => {
    try {
        ...
        const [inscripcion] = await pool.execute(`
          ...
          WHERE jtd.torneo_id = ? AND jtd.jugador_id = ?
        `, [torneoId, jugadorId]);
...
router.put('/:torneoId/actualizarInscripcion', verificarToken, upload.single('lista_ejercito'), async (req, res) => {
    ...
    try {
       ...
       let updateFields = ['nombre_banda = ?', 'banda = ?'];
        let updateValues = [nombre_banda, banda];
...
router.post('/:torneoId/add-individual-participant', verificarToken, verificarOrganizadorTorneo, async (req, res) => {
  ...
  try {
    ...
    // ✅ Insertar en jugador_torneo_dracula
    ...
    // ✅ Insertar en clasificacion_jugadores_dracula
    await connection.query(
      `INSERT INTO clasificacion_jugadores_dracula (
        torneo_id, 
        jugador_id, 
        partidas_jugadas, 
        partidas_ganadas, 
...
router.post('/:torneoId/jugadores/:jugadorId/reenviarInvitacionInd', verificarToken, verificarOrganizadorTorneo, async (req, res) => {
  ...
  try {
    ...
    const [jugadorData] = await connection.query(
      `SELECT jtd.id, jtd.usuario_id, u.nombre, u.apellidos, u.email, u.estado_cuenta, jtd.banda
       FROM jugador_torneo_dracula jtd
       INNER JOIN usuarios u ON jtd.usuario_id = u.id
       WHERE jtd.id = ? AND jtd.torneo_id = ?`,
      [jugadorId, torneoId]
    );
...
router.get('/:torneoId/verificarPagos', verificarToken, async (req, res) => {
    try {
        ...
        const [torneo] = await pool.execute(
            'SELECT id, tipo_torneo, created_by FROM torneos_sistemas WHERE id = ? AND sistema = "DRACULA"',
            [torneoId]
        );
...
router.delete('/:torneoId/eliminarTorneo', verificarToken, async (req, res) => {
  ...
  try {
    ...
    const [participantes] = await pool.execute(
      'SELECT COUNT(*) as total FROM jugador_torneo_dracula WHERE torneo_id = ?',
      [torneoId]
    );
...
router.delete('/:torneoId/jugadores/:jugadorId', verificarToken, async (req, res) => {
  try {
    ...
    const [participante] = await pool.execute(
      `SELECT jtd.id, jtd.jugador_id, u.nombre, u.apellidos 
       FROM jugador_torneo_dracula jtd
       INNER JOIN usuarios u ON jtd.jugador_id = u.id
       WHERE jtd.torneo_id = ? AND jtd.jugador_id = ?`,
      [torneoId, jugadorId]
    );
    ...
    await pool.execute(
      'DELETE FROM jugador_torneo_dracula WHERE torneo_id = ? AND jugador_id = ?',
      [torneoId, jugadorId]
    );
 ...
 //====================================================
  //METODOS PARA ACCEDER A JUGADORES DE LOS TORNEOS DRACULAS
  //====================================================
    ...
    try {
        ...
        const [jugadores] = await pool.execute(`
            SELECT 
                ...
                jtd.puntos_victoria,
                jtd.puntos_ronda,
                jtd.created_at as fecha_inscripcion
            FROM jugador_torneo_dracula jtd
            INNER JOIN usuarios u ON jtd.jugador_id = u.id
            WHERE jtd.torneo_id = ?
        `, [torneoId]);
...
// =====CAMBIAR ESTADO DEL TORNEO DRACULA=====
...
router.put('/:torneoId/estado', verificarToken, verificarOrganizadorTorneo, async (req, res) => {
  ...
  try {
    ...
    const [torneo] = await connection.query(
      'SELECT id, estado, nombre_torneo FROM torneos_sistemas WHERE id = ? AND sistema = ?',
      [torneoId, 'EPIC']
    );

    if (torneo.length === 0) {
      await connection.rollback();
      return res.status(404).json(errorResponse('Torneo DRACULA no encontrado'));
    }
    ...
    if (estadoActual === 'finalizado' && estado === 'finalizado') {
      await connection.rollback();
      return res.status(400).json(errorResponse('El torneo ya está finalizado'));
    }
    ...
    await connection.rollback();
    console.error('Error al cambiar estado del torneo DRACULA:', error.message);
    return res.status(500).json(errorResponse('Error al cambiar el estado del torneo'));
  } finally {
    connection.release();
  }
});
...
router.get('/:torneoId/partidasTorneoDracula', async (req, res) => {
  try {
    const { torneoId } = req.params;
    const { ronda } = req.query;
    
    let whereClause = 'WHERE pe.torneo_id = ?';
    let params = [torneoId];
    
    if (ronda) {
      whereClause += ' AND pe.ronda = ?';
      params.push(ronda);
    }
    ...
    const [partidas] = await pool.execute(`
      SELECT 
        ...
        pd.es_bye,
        pd.resultado_pd,
        pd.resultado_confirmado,
        pd.puntos_victoria_j1,
        pd.puntos_victoria_j2,
        pd.puntos_ronda_j1,
        pd.puntos_ronda_j2,
        pd.objetivos_cumplidos_j1,
        pd.objetivos_cumplidos_j2,
        pd.created_at,
        pd.fecha_partida,
        
        -- IDs de participación (ya son jugador_torneo_dracula.id)
        pd.jugador1_id,
        pd.jugador2_id,
        ...
        jt1.banda as jugador1_banda,
        
        -- Info Jugador 2
        jt2.jugador_id as jugador2_usuario_id,
        CASE 
          WHEN pe.es_bye = TRUE THEN NULL
          ELSE u2.nombre
        ...
        CASE 
          WHEN pe.es_bye = TRUE THEN NULL
          ELSE jt2.banda
        END as jugador2_banda
        
      FROM partidas_dracula pd
      
      -- JOINs: pd.jugador1_id = jugador_torneo_dracula.id (gracias a FK)
      INNER JOIN jugador_torneo_dracula jt1 ON pd.jugador1_id = jt1.id
      INNER JOIN usuarios u1 ON jt1.jugador_id = u1.id
      
      LEFT JOIN jugador_torneo_dracula jt2 ON pd.jugador2_id = jt2.id AND pd.es_bye = FALSE
      LEFT JOIN usuarios u2 ON jt2.jugador_id = u2.id
      
      ${whereClause}
      ORDER BY pd.ronda, pd.mesa, pd.id
    `, params);
    ...
    const partidasFormateadas = partidas.map(p => ({
      ...
      puntos_ronda_j1: p.puntos_ronda_j1,
      puntos_ronda_j2: p.puntos_ronda_j2,
      objetivos_cumplidos_j1: p.objetivos_cumplidos_j1,
      objetivos_cumplidos_j2: p.objetivos_cumplidos_j2,
      created_at: p.created_at,
      fecha_partida: p.fecha_partida,
      
      // IDs de participación (jugador_torneo_epic.id)
      ...
      jugador1: { 
        banda: p.jugador1_banda || null 
      },
      jugador2: p.jugador2_id ? { 
        banda: p.jugador2_banda || null 
      } : null
    }));
...
router.get('/:torneoId/partidasTorneoDracula/:partidaId', verificarToken, async (req, res) => {
  try {
    ...
    const [partidas] = await pool.execute(`
      SELECT 
        ...
        jtd1.banda as jugador1_banda,
        jtd2.banda as jugador2_banda,
        pd.ronda,
        ts.nombre_torneo
      FROM partidas_dracula pd
      JOIN usuarios u1 ON pd.jugador1_id = u1.id
      JOIN usuarios u2 ON pd.jugador2_id = u2.id
      JOIN torneos_sistemas ts ON pd.torneo_id = ts.id
      LEFT JOIN jugador_torneo_dracula jtd1 ON (pd.torneo_id = jtd1.torneo_id AND pd.jugador1_id = jtd1.jugador_id)
      LEFT JOIN jugador_torneo_dracula jtd2 ON (pd.torneo_id = jtd2.torneo_id AND pd.jugador2_id = jtd2.jugador_id)
      WHERE pd.id = ?
    `, [partidaId]);
...
router.put('/:torneoId/partidasTorneoDracula/:partidaId', verificarToken, async (req, res) => {
  try {
    const { partidaId, torneoId } = req.params;
    const { 
      puntos_masacre_j1,
      puntos_masacre_j2,
      objetivos_cumplidos_j1,
      objetivos_cumplidos_j2
    } = req.body;
    ...
    const [partida] = await pool.execute(`
      SELECT 
        pd.id,
        pd.jugador1_id,                         -- Ya es jugador_torneo_dracula.id
        pd.jugador2_id,                         -- Ya es jugador_torneo_dracula.id
        pd.resultado_pd, 
        pd.torneo_id, 
        pd.ronda, 
        pd.resultado_confirmado,
        ...
        ts.tipo_torneo
      FROM partidas_dracula pd
      INNER JOIN torneos_sistemas ts ON pd.torneo_id = ts.id
      WHERE pd.id = ? AND pd.torneo_id = ?
    `, [partidaId, torneoId]);
    ...
    const jugador2_id = partida[0].jugador2_id;  // jugador_torneo_epic.id
    ...
    const objetivosJ1 = parseInt(objetivos_cumplidos_j1) || 0;
    const objetivosJ2 = parseInt(objetivos_cumplidos_j2) || 0;

    let puntosVictoriaJ1, puntosVictoriaJ2, resultado;

     if (objetivosJ1 > objetivosJ2) {
      puntosVictoriaJ1 = 3;
      puntosVictoriaJ2 = 0;
      resultado = 'victoria_j1';
    } else if (objetivosJ2 > objetivosJ1) {
      puntosVictoriaJ1 = 0;
      puntosVictoriaJ2 = 3;
    ...
    res.status(200).json(
      successResponse('Partida registrada exitosamente (pendiente de confirmación)', {
        partidaId,
        resultado,
        puntosVictoria: { jugador1: puntosVictoriaJ1, jugador2: puntosVictoriaJ2 },
        puntosMasacre: { jugador1: puntosMasacreJ1, jugador2: puntosMasacreJ2 },
        objetivosCumplidos: { jugador1: objetivosJ1, jugador2: objetivosJ2 }
      })
    );
...
router.patch('/:torneoId/partidasTorneoEpic/:partidaId/confirmar', verificarToken, async (req, res) => {
  let connection;
  ...
  try {
    ...
    const [verificacion] = await connection.execute(
      `SELECT 
        ts.created_by,
        pe.id, 
        pe.jugador1_id as participacion_j1_id,     -- jugador_torneo_epic.id
        pe.jugador2_id as participacion_j2_id,     -- jugador_torneo_epic.id
        jt1.jugador_id as jugador1_id,             -- usuarios.id
        jt2.jugador_id as jugador2_id,             -- usuarios.id
        pe.puntos_victoria_j1, 
        pe.puntos_victoria_j2,
        pe.puntos_masacre_j1, 
        pe.puntos_masacre_j2,
        COALESCE(pe.objetivos_cumplidos_j1, 0) as objetivos_cumplidos_j1,
        COALESCE(pe.objetivos_cumplidos_j2, 0) as objetivos_cumplidos_j2,
        pe.resultado_confirmado,
        pe.resultado_pe,
        pe.es_bye
      FROM torneos_sistemas ts
      INNER JOIN partidas_epic pe ON pe.torneo_id = ts.id
      LEFT JOIN jugador_torneo_epic jt1 ON pe.jugador1_id = jt1.id
      LEFT JOIN jugador_torneo_epic jt2 ON pe.jugador2_id = jt2.id
      WHERE ts.id = ? AND pe.id = ?`,
      [torneoId, partidaId]
    );
    ...
    if (esBye) {
      ...
      switch (partidaData.resultado_pe) {
        case 'victoria_j1':
          j1Gana = 1;
          j2Pierde = 1;
          break;
    ...
    if (confirmar) {
        // Actualizar jugador_torneo_dracula (usa participacion_id)
      ...
      await connection.execute(`
        ...
        ON DUPLICATE KEY UPDATE
          ...
          puntos_ronda_totales = puntos_ronda_totales + VALUES(puntos_ronda_totales)
      `, [
        torneoId, 
      ...
      await connection.execute(`
        ...
        SET puntos_victoria = GREATEST(0, puntos_victoria - ?),
            puntos_ronda = GREATEST(0, puntos_ronda - ?)
        WHERE id = ? AND torneo_id = ?
      `, [
        partidaData.puntos_victoria_j1 || 0,
        partidaData.puntos_ronda_j1 || 0,
      ...
      await connection.execute(`
        UPDATE clasificacion_jugadores_dracula 
        SET 
          ...
          puntos_ronda_totales = GREATEST(0, puntos_ronda_totales - ?)
        WHERE torneo_id = ? AND jugador_id = ?
      `, [
        j1Gana,
        j1Empata,
    ...
    await connection.execute(
      'UPDATE partidas_dracula SET resultado_confirmado = ? WHERE id = ?',
      [confirmar, partidaId]
    );
...
router.get('/:torneoId/obtenerEmparejamientosIndividuales', verificarToken, async (req, res) => {
  try {
    ...
    const queryConJoins = `
      SELECT 
        pe.*,
        jtd1.jugador_id as jugador1_usuario_id,
        jtd2.jugador_id as jugador2_usuario_id,
        u1.nombre as jugador1_nombre,
        u1.apellidos as jugador1_apellidos,
        u2.nombre as jugador2_nombre,
        u2.apellidos as jugador2_apellidos
      FROM partidas_dracula pd
      INNER JOIN jugador_torneo_dracula jtd1 ON pd.jugador1_id = jtd1.id
      LEFT JOIN jugador_torneo_dracula jtd2 ON pd.jugador2_id = jtd2.id AND pd.es_bye = FALSE
      INNER JOIN usuarios u1 ON jtd1.jugador_id = u1.id
      LEFT JOIN usuarios u2 ON jtd2.jugador_id = u2.id
      ${whereClause}
      ORDER BY pd.mesa, pd.id
    `;
...
router.post('/:torneoId/guardarEmparejamientosIndividuales', verificarToken, async (req, res) => {
  ...
  try {
    ...
    await connection.execute(
      'DELETE FROM partidas_dracula WHERE torneo_id = ? AND ronda = ?',
      [torneoId, ronda]
    );
    ...
    for (const partida of emparejamientos) {
      ...
      const [j1] = await connection.execute(
        'SELECT id FROM jugador_torneo_dracula WHERE jugador_id = ? AND torneo_id = ?',
      ...
      await connection.execute(insertQuery, [
        torneoId,
        jugador1_participacion_id,  // FK a jugador_torneo_epic.id
        jugador2_participacion_id,  // FK a jugador_torneo_epic.id
        ronda,
        partida.mesa || null,
...
router.delete('/:torneoId/partidasTorneoDracula/:partidaId', verificarToken, async (req, res) => {
  try {
    const { partidaId } = req.params;
    
    // Verificar que la partida existe y permisos
    const [partidaExistente] = await pool.execute(`
      SELECT pd.*, ts.created_by
      FROM partidas_dracula pd
      JOIN torneos_sistemas ts ON pd.torneo_id = ts.id
      WHERE pd.id = ?
    `, [partidaId]);
    ...
    // Eliminar la partida
    await pool.execute('DELETE FROM partidas_dracula WHERE id = ?', [partidaId]);
     ...
     const { torneoId } = req.params;
        ...
        const [clasificacion] = await pool.execute(`
            SELECT 
                cjd.id,
                cjd.jugador_id,
                cjd.partidas_ganadas,
                cjd.partidas_empatadas,
                cjd.partidas_perdidas,
                u.nombre as jugador_nombre,
                u.apellidos as jugador_apellidos,
                u.club,
                jte.ejercito,
                COALESCE(cjd.partidas_jugadas, 0) as partidas_jugadas,
                 COALESCE(cjd.partidas_ganadas, 0) as jugador_partidas_ganadas,
                COALESCE(cjd.partidas_empatadas, 0) as jugador_partidas_empatadas,
                COALESCE(cjd.partidas_perdidas, 0) as jugador_partidas_perdidas,
                COALESCE(cjd.puntos_victoria_totales, 0) as puntos_victoria_totales,
                COALESCE(cjd.puntos_masacre_totales, 0) as puntos_masacre_totales
              FROM clasificacion_jugadores_dracula cjd
                INNER JOIN usuarios u 
                  ON cjd.jugador_id = u.id
                LEFT JOIN jugador_torneo_dracula jtd
                  ON cjd.jugador_id = jtd.jugador_id
                  AND cjd.torneo_id = jtd.torneo_id 
              WHERE cjd.torneo_id = ?
        `, [torneoId]);
...
// ======= ENDPOINTS PARA CORREOS - DRACULA =======
...
router.get('/:torneoId/listasEjercitos-pdf/:jugadorId', verificarToken, async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(
      'SELECT lista_ejercito, lista_nombre FROM jugador_torneo_dracula WHERE torneo_id = ? AND jugador_id = ?',
      [torneoId, jugadorId]
    );
