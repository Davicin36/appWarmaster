...
CREATE TABLE jugador_torneo_epic (
...
CREATE TABLE clasificacion_jugadores_epic (
  id INT PRIMARY KEY AUTO_INCREMENT,
  torneo_id INT NOT NULL,
  jugador_id INT NOT NULL,
...
CREATE TABLE partidas_epic (
  ...
  objetivos_cumplidos_j1 INT DEFAULT 0,
  objetivos_cumplidos_j2 INT DEFAULT 0,
  resultado_pe ENUM('victoria_j1', 'victoria_j2', 'empate', 'pendiente') DEFAULT 'pendiente',
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (torneo_id) REFERENCES torneos_sistemas(id) ON DELETE CASCADE,
  FOREIGN KEY (jugador1_id) REFERENCES jugador_torneo_epic(id),
  FOREIGN KEY (jugador2_id) REFERENCES jugador_torneo_epic(id)
);

/**
CREATE TABLE torneo_epic_equipo(
  id INT PRIMARY KEY AUTO_INCREMENT,
  torneo_id INT NOT NULL,
...
CREATE TABLE  equipo_miembros (
  ...
  FOREIGN KEY (equipo_id) REFERENCES torneo_epic_equipo(id) ON DELETE CASCADE,
  FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  FOREIGN KEY (jugador_eq_id) REFERENCES jugador_torneo_epic(id) ON DELETE CASCADE,
  UNIQUE KEY unique_equipo_usuario_jugador (equipo_id, usuario_id, jugador_eq_id)
);


CREATE TABLE clasificacion_equipos_epic (
  id INT PRIMARY KEY AUTO_INCREMENT,
  torneo_id INT NOT NULL,
  equipo_id INT NOT NULL,
  partidas_jugadas INT DEFAULT 0, 
