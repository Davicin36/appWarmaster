...
CREATE TABLE jugador_torneo_dracula (
  id INT PRIMARY KEY AUTO_INCREMENT,
  torneo_id INT NOT NULL,
  jugador_id INT NOT NULL,
  banda VARCHAR(100) NULL DEFAULT NULL,
  nombre_banda VARCHAR(200) NULL DEFAULT NULL,
...
CREATE TABLE clasificacion_jugadores_dracula (
  id INT PRIMARY KEY AUTO_INCREMENT,
  torneo_id INT NOT NULL,
  jugador_id INT NOT NULL,
  ...
  puntos_ronda_totales INT DEFAULT 0,
  FOREIGN KEY (torneo_id) REFERENCES torneos_sistemas(id) ON DELETE CASCADE,
  FOREIGN KEY (jugador_id) REFERENCES usuarios(id) ON DELETE CASCADE,
...
CREATE TABLE partidas_dracula (
  ...
  puntos_ronda_j1 INT DEFAULT 0,
  puntos_ronda_j2 INT DEFAULT 0,
  resultado_pd ENUM('victoria_j1', 'victoria_j2', 'empate', 'pendiente') DEFAULT 'pendiente',
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (torneo_id) REFERENCES torneos_sistemas(id) ON DELETE CASCADE,
  FOREIGN KEY (jugador1_id) REFERENCES jugador_torneo_dracula(id),
  FOREIGN KEY (jugador2_id) REFERENCES jugador_torneo_dracula(id)
);

/**
CREATE TABLE torneo_dracula_equipo(
  id INT PRIMARY KEY AUTO_INCREMENT,
  torneo_id INT NOT NULL,
  nombre_equipo VARCHAR(150),
  capitan_id INT NOT NULL,
  puntos_victoria_equipo INT DEFAULT 0,
  puntos_ronda_equipo INT DEFAULT 0,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE  equipo_miembros (
  ...
  FOREIGN KEY (equipo_id) REFERENCES torneo_dracula_equipo(id) ON DELETE CASCADE,
  FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  FOREIGN KEY (jugador_eq_id) REFERENCES jugador_torneo_dracula(id) ON DELETE CASCADE,
  UNIQUE KEY unique_equipo_usuario_jugador (equipo_id, usuario_id, jugador_eq_id)
);


CREATE TABLE clasificacion_equipos_dracula (
  id INT PRIMARY KEY AUTO_INCREMENT,
  torneo_id INT NOT NULL,
  equipo_id INT NOT NULL,
  partidas_jugadas INT DEFAULT 0, 
  ...
  puntos_ronda_eq_totales INT DEFAULT 0,
  FOREIGN KEY (torneo_id) REFERENCES torneos_sistemas(id) ON DELETE CASCADE,
  FOREIGN KEY (equipo_id) REFERENCES torneo_dracula_equipo(id) ON DELETE CASCADE,
