...
function PrincipalFow({ onOpenLogin }) {
    const navigate = useNavigate();
    const { isAuthenticated, user} = useAuth();

    const userId = user?.id || null;
    
    const [torneosFow, setTorneosFow] = useState([]);
    
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState('');

    const obtenerTorneosFow= async () => {
        try {
            setLoading(true);
            setError('');
           ...
           const token = localStorage.getItem('token');
            ...
            const data = await torneosFowApi.obtenerTorneos();
            
            if (data.data && data.data.torneosFow) {
                setTorneosFow(data.data.torneosFow);
            } else {
    ...
    return (
        <div>      
            ...
            {error && (
             ...
             <section>
                <img src={logoFow} alt="logo de FOW"  />
                                ...
                                {torneosFow.map((torneo, index) => (
                                    <tr key={torneo.id}>
                                        ...
                                        <td>
                                            ...
                                            {torneo.total_participantes > 0 && (
                                                <div className="bandos_desglose">
                                                    <small className="bando-eje">
                                                        Eje: {torneo.total_eje || 0} 
                                                    </small>
                                                    <small className="bando-aliados">
                                                        Aliados: {torneo.total_aliados || 0} 
                                                    </small>
                                                </div>
                                            )}
                                        </td>
                                        <td className="acciones-cell">
                                                ...
                                                <button 
                                                    ...
                                                    onClick={() => {
                                                        if (torneo.usuario_inscrito) {
                                                            navigate(`/torneosFow/${torneo.id}/editar-inscripcion`);
                                                        } else {
                                            ...
                                            <button 
                                                ...
                                                onClick={() => navigate(`/torneosFow/${torneo.id}/detalles`)}
                                            >   
                                                👁️ Ver Detalles
                                            </button>
                                        </td>
